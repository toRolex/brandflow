...
class ScriptGenerator:
    ...
    def _mock_result(
        ...
        if language == "cantonese":
            first = (
                f"呢款{product}選用上等原料精心製作而成，每一道工序都經過嚴格把控，"
                f"品質始終穩定如一，從選材到成品每一個環節都精益求精。"
                f"無論日常使用定係特殊場合，佢都可以為你帶嚟出色嘅使用體驗，"
                f"用過你就知道佢嘅獨特之處"
            )
            second = (
                f"{brand}深耕行業多年，始終堅持高標準嚴要求，"
                f"以用戶為中心不斷優化迭代升級，贏得咗廣大用戶嘅信賴同一致好評。"
                f"而家落單仲可以享受專屬優惠活動，機會非常難得千祈唔好錯過，"
                f"快啲行動將佢帶返屋企啦"
            )
        else:
            first = (
                f"这款{product}选用上等原料精心制作而成，每一道工序都经过严格把控，"
                f"品质始终稳定如一，从选材到成品每一个环节都精益求精。"
                f"无论是日常使用还是特殊场合，它都能为你带来出色的使用体验，"
                f"用过你就知道它的独特之处"
            )
            second = (
                f"{brand}深耕行业多年，始终坚持高标准严要求，"
                f"以用户为中心不断优化迭代升级，赢得了广大用户的信赖和一致好评。"
                f"现在下单还能享受专属优惠活动，机会非常难得千万不要错过，"
                f"赶紧行动把它带回家吧"
            )
        full = first + "。" + second + "。"
        if language == "cantonese":
