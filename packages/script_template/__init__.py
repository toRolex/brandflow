"""Script template — template models, rendering, and JSON file store."""
...
def render_template(
    ...
    """Render a template by filling slots and substituting variables."""
...
class ScriptTemplateStore:
    """ScriptTemplate store — one JSON file per template under templates_dir."""
    ...
    def create_template(self, template: ScriptTemplate) -> ScriptTemplate:
        if not template.id:
            template.id = f"tmpl_{uuid.uuid4().hex[:12]}"
