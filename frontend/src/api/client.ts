...
export const api = {
	...
	getProject: (id: string) =>
		request<
			import("../types").Project & { jobs: import("../types").JobSummary[] }
		>(`/api/projects/${id}`),
	...
	uploadAsset: (projectId: string, file: File) =>
		uploadFile<import("../types").AssetFile>(
			`/api/projects/${projectId}/upload`,
			file,
		),

	listAssets: (projectId: string) =>
		request<import("../types").AssetFile[]>(
			`/api/projects/${projectId}/assets`,
		),

	// Asset Library
	listIndexedAssets: async (
		projectId: string,
	...
	indexAssets: (projectId: string) =>
		request<import("../types").IndexResult>(
			`/api/projects/${projectId}/assets/index`,
			{
				method: "POST",
			},
		),
	...
	listIndexedAssetsShared: async (params?: {
		category?: string;
		q?: string;
		product?: string;
	}) => {
	...
	indexAssetsShared: () =>
		request<import("../types").IndexResult>("/api/assets/index", {
			method: "POST",
		}),
	...
	getIndexStatus: (taskId: string) =>
		request<import("../types").IndexTaskState>(
			`/api/assets/index/${taskId}/status`,
		),
	...
	createJob: (
		...
		body: {
			...
			platforms: string[];
			name?: string;
			mode?: import("../types").ProductionMode;
		...
		request<import("../types").JobDetail>(
	...
	batchCreateJobs: (
		projectId: string,
		body: import("../types").BatchCreateRequest,
	) =>
		request<import("../types").BatchCreateResponse>(
			`/api/projects/${projectId}/jobs/batch`,
			{
				method: "POST",
				body: JSON.stringify(body),
			},
	...
	editScript: (jobId: string, scriptText: string, projectId?: string) => {
		const qs = projectId ? `?project_id=${projectId}` : "";
		return request<{ status: string }>(
			`/api/reviews/${jobId}/edit-script${qs}`,
			{
				method: "POST",
	...
	// Config
	getConfig: () => request<import("../types").ProviderConfig>("/api/config"),
	...
	getProductConfigById: (productId: string) =>
		request<import("../types").ProductConfig>(
			`/api/products/${productId}/config`,
		),

	saveProductConfigById: (
		productId: string,
		payload: import("../types").ProductConfig,
	) =>
		request<import("../types").ProductConfig>(
			`/api/products/${productId}/config`,
			{
	...
	scanMetrics: () =>
		request<import("../types").ScanResult>("/api/metrics/scan", {
			method: "POST",
		}),
	...
	getMetricsOverview: (days: number = 7, platform?: string) => {
		...
		return request<import("../types").MetricsOverview>(
	...
	getMetricsVideos: (params?: {
		...
		return request<import("../types").VideoMetricPage>(
	...
	getMetricsTopics: (
		...
		return request<import("../types").TopicStat[]>(
			`/api/metrics/topics?${qs.toString()}`,
		);
	},
	...
	previewTemplate: (
		...
		request<import("../types").PreviewResponse>(
			`/api/config/templates/${id}/preview`,
			{
				method: "POST",
				body: JSON.stringify({
	...
	suggestCategories: () =>
		request<{
			suggestions: import("../types").SuggestCategory[];
			errors: string[];
