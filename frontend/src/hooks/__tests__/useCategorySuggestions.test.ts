...
const MOCK_SUGGESTIONS: SuggestCategory[] = [
	{
		label: "产品细节",
		description: "产品特写展示",
		vision_prompt: "product detail close-up",
	},
	{
		label: "调味过程",
		description: "加入调料的过程",
...
describe("useCategorySuggestions", () => {
	...
	it("S2: handleSuggest 网络失败设置错误", async () => {
		vi.mocked(api.suggestCategories).mockRejectedValue(
	...
	it("S3: handleSuggest API 返回 errors 时设置错误", async () => {
		...
		expect(result.current.suggestError).toBe(
			"未找到可用素材；Vision API 调用失败",
		);
		expect(result.current.suggestLoading).toBe(false);
	});
		...
		const existingCategories: CategoryConfig[] = [
			{
				...
				name: "产品细节",
				description: "已有",
				vision_prompt: "existing",
			},
		];
		const { result } = renderHook(() =>
			useCategorySuggestions(existingCategories, onConfirm),
		);
		onConfirm.mockResolvedValue(undefined);
	...
	it("S14: 先有后端错误时网络失败不覆盖 suggestError", async () => {
		...
		vi.mocked(api.suggestCategories).mockRejectedValueOnce(
			new Error("Network Error"),
		);
