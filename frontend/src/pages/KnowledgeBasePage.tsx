  ...
  return (
    <div>
      ...
      <div className="border-2 border-dashed rounded-lg p-6 mb-6 text-center" style={{ borderColor: "var(--border-default)" }}>
        ...
        <input ref={fileRef} type="file" accept=".txt,.pdf,.docx" className="block mx-auto text-sm" style={{ color: "var(--text-secondary)" }} />
        <button onClick={upload} disabled={uploading}
          className="mt-3 px-4 py-1.5 text-sm font-medium rounded-md transition-colors disabled:opacity-50"
          style={{ background: "var(--btn-primary-bg)", color: "var(--text-inverse)" }}>{uploading ? "上传中..." : "上传并解析"}</button>
      </div>
