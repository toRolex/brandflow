...
const MOCK_CATEGORIES = [
	{
		...
		vision_prompt: "product showcase close-up",
	},
	{
		id: "qianqizhunbei",
		name: "前期准备",
		description: "产品前期的整理、检查等准备过程",
		vision_prompt: "preparation inspection organization",
	},
	{
		id: "zhizuoguocheng",
		name: "制作过程",
		description: "产品的加工和制作过程",
		vision_prompt: "production manufacturing process",
	},
];
...
describe("CategoryManager", () => {
		...
		vi.mocked(api.saveProductConfig).mockResolvedValue({
			...
			categories: [
				...MOCK_CATEGORIES,
				{
					...
					name: "成品出锅",
					description: "最终成品展示",
					vision_prompt: "final dish plating",
				},
			],
		});
			...
			suggestions: [
				{
					label: "产品细节",
					description: "产品特写",
					vision_prompt: "product display",
				},
				{
					label: "调味过程",
					description: "加入调料",
					vision_prompt: "seasoning process",
				},
			],
			errors: [],
		});
	...
	it("AI 建议返回 errors 时显示错误提示", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("未找到可用素材；Vision API 调用失败"),
			).toBeInTheDocument();
		});
	});
...
describe("CategoryManager - id handling", () => {
	...
	it("S6: 新增分类时自动生成非空 id", async () => {
		...
		const categories =
			(savedPayload as { categories?: Array<{ id: string }> })?.categories ??
			[];
	...
	it("S7: 编辑分类名称不改变已有 id", async () => {
		...
		const categories =
			(savedPayload as { categories?: Array<{ id: string; name: string }> })
				?.categories ?? [];
