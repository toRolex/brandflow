...
describe("InlineBanner", () => {
	...
	it("renders error message with correct styling", () => {
		render(<InlineBanner type="error" message="操作失败" onClose={vi.fn()} />);
	...
	it("calls onClose when close button is clicked", () => {
		const onClose = vi.fn();
		render(<InlineBanner type="success" message="测试" onClose={onClose} />);
	...
	it("does not auto-hide error by default", () => {
		vi.useFakeTimers({ shouldAdvanceTime: true });
		const onClose = vi.fn();
		render(<InlineBanner type="error" message="错误信息" onClose={onClose} />);
		act(() => {
