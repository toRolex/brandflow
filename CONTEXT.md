...
### ConfigReader
纯配置读取器，带构建期热缓存。读取 `config/app_config.json`，合并 `packages/provider_config/config_constants.py` 中的 `DEFAULTS`，并按 product 应用覆盖。所有 `get_*()` 方法在构建后均为 O(1) 字典查找。路径：`packages/provider_config/config_reader.py`。

### ProductStore
product 级配置的 CRUD 存储。与 `ConfigReader` 协同，所有写入通过 `config_io.save_config()` 持久化，并调用 `ConfigReader.reload()` 刷新热缓存。支持 product 创建、切换、重命名、删除，以及 product 级配置覆盖写入。路径：`packages/provider_config/product_store.py`。
...
### Provider（服务提供者）
AI 能力的供应商。LLM 支持 deepseek（默认 `deepseek-v4-pro`）/ kimi / openai；TTS 默认 qwen，同时支持 mimo（minimax 代码已废弃）；Vision 支持 xiaomi（默认 `mimo-v2.5`）/ claude。每个 Provider 有独立的环境变量命名规则。

### Legacy Bridge（遗留桥接层）
`LegacyScriptBridge`（`packages/pipeline_services/legacy_script_bridge.py`）是目前仅存的遗留桥接层，用于在 `PhaseOrchestrator` 与 `ScriptGenerator` 之间保持兼容的调用约定。`LegacyMediaBridge`、`LegacyScheduleBridge` 以及被包装的旧核心文件（`main_controller.py`、`kimi_two_stage_script.py`）已在 v0.7.0 重构中删除。

## 架构状态（v0.7.0）

v0.7.0 已完成控制面 + 执行器 + pipeline service 的分层重构，配置体系由 `ConfigReader` / `ProductStore` / `SecretStore` / `DEFAULTS` 接管，`PhaseOrchestrator` 统一调度 10 个 phase handler。

### 已确认废弃的能力
- `AppConfigManager` — 已被 `ConfigReader` + `ProductStore` + `SecretStore` + `DEFAULTS` 替代
