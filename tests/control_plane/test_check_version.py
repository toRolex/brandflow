...
def test_check_version_returns_expected_shape(client) -> None:
...
def test_check_version_detects_new_version(client) -> None:
...
def test_check_version_silent_on_github_failure(client) -> None:
...
def test_check_version_no_update_when_same(client) -> None:
...
def test_update_normal_path(client) -> None:
...
def test_update_concurrent_blocked(client) -> None:
    """已有更新进程时第二次请求返回 409。"""
...
def test_update_non_windows(client) -> None:
    """非 Windows 平台返回可读错误。"""
    version_check._update_in_progress = False
    with patch(
