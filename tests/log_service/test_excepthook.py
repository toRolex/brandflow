...
def test_reinstall_wraps_a_new_active_delegate(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr("packages.log_service.log_writer.get_log_dir", lambda: tmp_path)
    first_calls = []
    second_calls = []
    monkeypatch.setattr(sys, "excepthook", lambda *args: first_calls.append(args))
    install_global_excepthook()
    install_global_excepthook()
    monkeypatch.setattr(sys, "excepthook", lambda *args: second_calls.append(args))

    install_global_excepthook()
    error = RuntimeError("new delegate")
    sys.excepthook(type(error), error, error.__traceback__)
