...
# GET /api/update/status            — 查询更新进度，返回 progress.json 状态（含 stalled 判定）
# POST /api/update                  — 一键更新（Windows），异步执行 update.bat

# 测试
uv run pytest tests/ -q                # 全量测试
uv run pytest tests/ -q -n 2           # 并行（xdist）减少内存开销

# 内存泄漏检测
uv run pytest tests/ -q --memray       # 每个测试的高水位内存分配报告
uv run pytest tests/ -q -n 2          # 推荐：进程隔离避免泄漏累积
