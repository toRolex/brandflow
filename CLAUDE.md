...
| 零食公司 Win10（`100.81.135.74` / `192.168.1.224`） | `admin` | `D:\brandflow` |
...
# 零食公司 Win10（Tailscale 跨网）
Host snack
    HostName 100.81.135.74
    User admin
...
# 零食公司 Win10（Tailscale 跨网）
ssh snack "cmd /c \"cd /d D:\brandflow && <命令>\""

# 零食公司 Win10（局域网直连）
ssh snack-lan "cmd /c \"cd /d D:\brandflow && <命令>\""
```
...
| 零食公司 Win10 | `DESKTOP-9H34LPA` | `100.81.135.74` | Tailscale 虚拟 IP，跨网段可用 |
| 零食公司 Win10 | `DESKTOP-9H34LPA` | `192.168.1.224` | 局域网直连 IP |

**零食公司 Win10 部署信息：**
| 项目 | 信息 |
|------|------|
| 系统 | Windows 10 专业版 22H2（10.0.19045） |
| 用户 | `admin` |
| 项目路径 | `D:\brandflow`（clone 自 `toRolex/brandflow`） |
| 素材路径 | `D:\物料文件\` |
| 工具 | Git 2.52.0 / uv 0.9.25 / Python 3.11.9 / FFmpeg 8.0.1 |
...
# Web 健康接口：附加 ?deploy_check=true 返回详细体检结果
curl "http://127.0.0.1:17890/api/health?deploy_check=true"
```
...
| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/health` | GET | 健康检查；`?deploy_check=true` 返回部署体检结果 |
...
│   ├── file_store/                   # 文件系统轻持久化
│   ├── deploy_health/                # 部署体检：CLI + Web 健康接口（Issue #76）
│   │   ├── __init__.py
│   │   ├── __main__.py               # CLI 入口
│   │   └── checker.py                # DeployHealthChecker
