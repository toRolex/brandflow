...
**Register:** product（管理后台/工具型界面）

**设计系统文件:**
- `PRODUCT.md` — 产品战略：用户、品牌个性、设计原则
- `DESIGN.md` — 视觉规范：色板、字体、组件、Named Rules（规范源）
- `.impeccable/design.json` — 组件 sidecar（live panel 渲染用）

**核心约束（AI 生成 UI 时务必遵守）:**
- **The One Accent Rule:** `electric-blue` (oklch(62% 0.19 252)) 覆盖 ≤10% 面积，仅用于链接/选中/主操作
- **The Cold Neutral Rule:** 所有中性色色相统一偏蓝轴 (255-258°)，禁止暖调中性色（奶油/沙色/米色）
- **The Signal-Only Rule:** `signal-green`/`alert-red`/`caution-amber` 仅用于状态传递，禁止装饰
- **The System Font Rule:** 系统字体栈，不引入 Web 字体
- **The Flat-By-Default Rule:** 静止表面为平，阴影仅在浮层出现
- **The Calculated Z-Index Rule:** z-index 从语义刻度选取 (100-600)，禁止 999/9999

**反参考（禁止）:** 暖调中性色背景、侧边彩色竖线、渐变文字、玻璃态、tiny uppercase eyebrow、01/02/03 数字脚手架、等大卡片网格

**暗色优先:** 暗色模式 (`void`/`obsidian`) 为设计原点，亮色模式 (`chalk`/`frost`) 同等质量
